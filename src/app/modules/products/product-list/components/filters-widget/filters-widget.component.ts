...
export class FiltersWidgetComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    if (this.filters) {
      ...
      const SUBCATEGORY = this.filters.find(filter => filter.type == 'subcategory');
      ...
      this.form.patchValue({
        ...
        subcategory: (SUBCATEGORY && SUBCATEGORY.value.length) ? SUBCATEGORY.value[0] : null,
  ...
  changeSelectedCategory(categoryId: any) {
    let queryParams = { ...this._activatedRoute.snapshot.queryParams };
    if (this.form.value['category'] == categoryId && !this.form.value['subcategory']) {
      this.form.patchValue({ category: null });
      delete queryParams['categoria'];
    } else {
      this.form.patchValue({ subcategory: null });
      delete queryParams['subcategoria'];
      this.form.patchValue({ category: categoryId });
      queryParams['categoria'] = this.form.value['category'];
    }

    this._router.navigate([], { queryParams });
  }

  changeSelectedSubcategory(categoryId: any, subcategoryId: any) {
    let queryParams = { ...this._activatedRoute.snapshot.queryParams };
    if (this.form.value['subcategory'] == subcategoryId) {
      this.form.patchValue({ subcategory: null });
      delete queryParams['subcategoria'];
    } else {
      this.form.patchValue({ category: categoryId });
      queryParams['categoria'] = this.form.value['category'];
      this.form.patchValue({ subcategory: subcategoryId });
      queryParams['subcategoria'] = this.form.value['subcategory'];
    }
